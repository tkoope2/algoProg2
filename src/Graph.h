...
class graph {
  ...
  public:
     ...
     *
     */   
    // * &report contains contains the tree structure we are working on
    // * src is the starting node sent over in positon value
    bool bfs(int src, std::vector<vertex_label> &report) {
      ...
      report[src].npaths = 1;//? Initalize path count
      q.push(src);
      ...
      /*
        How is it determined if it is discovered or not
          - If discovered 1
          - Not 0

        for loop should count number of hops
      */
